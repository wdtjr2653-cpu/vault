...
- [ ] **[검토] λ 붕괴 파일의 DRT 유래 피처 오염** — N-9/16/21/29는 λ*가 그리드 최소 근처(3.7e-7)로 떨어져 DRT가 스파이크화, peak1_gamma 등 DRT 유래 피처가 왜곡된 채 학습에 들어감(측정 자체는 정상). 현재 오탐과는 무관하나(반복 10회 중 1회 이하) λ 최소측 경계 플래그(현행 lam_grid_boundary_hit은 최대측만) 추가 검토
- [ ] **[검토] 상습 오탐 N-24·N-8** — 반복 10회 중 9회 불량 오판. 육안 결과 측정 정상이며 저주파 확산 질량이 커서 불량 패턴과 유사(N-24는 고주파 미세 루프로 NYQ outlier이기도). 라벨 재확인(정말 정상 셀인지) 또는 수용(오탄 4건 중 2건이 여기서 나옴) — 측정팀 협의 후보
