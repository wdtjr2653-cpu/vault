...
- [ ] **[검토] 상습 오탐 N-24·N-8** — 반복 10회 중 9회 불량 오판. 육안 결과 측정 정상이며 저주파 확산 질량이 커서 불량 패턴과 유사(N-24는 고주파 미세 루프로 NYQ outlier이기도). 라벨 재확인(정말 정상 셀인지) 또는 수용(오탄 4건 중 2건이 여기서 나옴) — **측정팀 문의 문안 작성 완료(2026-09-01), 발송 대기**(수신자 확정 필요, 저주파 편중·반복측정 프로토콜 문의와 통합 문안)
- [ ] 배치 재실행하여 `summary.csv`/`validation_summary.csv`에 λ 붕괴 컬럼·WARN 반영 (아래 완료 항목의 새 플래그는 코드에만 있고 기존 CSV엔 아직 없음)
