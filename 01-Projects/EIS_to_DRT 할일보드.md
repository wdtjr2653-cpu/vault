...
- [ ] **[검토] 상습 오탐 N-24·N-8** — 반복 10회 중 9회 불량 오판. 육안 결과 측정 정상이며 저주파 확산 질량이 커서 불량 패턴과 유사(N-24는 고주파 미세 루프로 NYQ outlier이기도). 라벨 재확인(정말 정상 셀인지) 또는 수용(오탄 4건 중 2건이 여기서 나옴) — **측정팀 없음 확인(2026-09-02, 사용자) — 외부 문의 불가, 데이터만으로 처리**: 라벨 외부 재확인이 불가하므로 '수용' 방향(오탐 4건 중 2건 발생원). 처리 옵션: (a) 현행 유지+경계성 셀로 문서화, (b) 학습 제외 후 성능 비교, (c) τ shift 등 신규 피처로 판별력 개선 시 자연 해소 여부 확인
